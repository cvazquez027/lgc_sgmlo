...
export default function MatricesPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    version: 1,
    id_estado_matriz: "1", // Por defecto: 1 (Borrador)
    vigente: 1
  });
  ...
  // Carga de establecimientos dinámicos (Filtro)
  useEffect(() => {
    if (!filtroCliente) {
  ...
  // Función auxiliar para el Modal: Carga establecimientos y los retorna
  ...
  const openCrearModal = () => {
    ...
    setFormData({
      ...
      fecha_desde: new Date().toISOString().split('T')[0],
      version: 1,
      id_estado_matriz: "1", // Siempre nace en Borrador (1)
      vigente: 1
    });
    setIsModalOpen(true);
  };
  ...
  const openEditarModal = async (matriz: Matriz) => {
    ...
    // Precargar cliente y establecimientos
    ...
    setFormData({
      ...
      id_estado_matriz: matriz.id_estado_matriz?.toString() || "1", // Mantenemos su estado actual
  ...
  // Filtrado local en base a los combos
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      {/* HEADER COMPACTO Y FILTROS */}
      <div className="bg-white px-5 py-3 rounded-xl shadow-sm border border-slate-100 flex flex-col gap-3">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <h1 className="text-xl font-heading text-lgc-primary uppercase tracking-tight">Matrices Legales</h1>
          {canEdit("matriz") && (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden min-h-100 flex flex-col">
        {loading ? (
           <div className="flex-1 flex items-center justify-center py-20 text-slate-400 animate-pulse font-heading">
            ...
            <table className="w-full text-left font-sans">
              ...
              <tbody className="divide-y divide-slate-50">
                {matricesFiltradas.length === 0 ? (
                  <tr><td colSpan={7} className="p-12 text-center text-slate-400 font-bold uppercase text-xs">No se encontraron matrices con los filtros actuales.</td></tr>
                ) : (
                  matricesFiltradas.map(matriz => (
                    <tr key={matriz.id_matriz} className="hover:bg-slate-50/50 transition-colors group">
                      
                      {/* COLUMNA: CLIENTE (LOGO + NOMBRE) */}
                      <td className="p-4">
                      ...
                      {/* COLUMNA: TIPO (SIN CORTE DE LÍNEA) */}
                      ...
                      {/* COLUMNA: ESTABLECIMIENTO REAL */}
                      <td className="p-4">
                        <div className="font-bold text-slate-800 text-xs uppercase">
                          {matriz.establecimiento_desc || `ID: ${matriz.id_cliente_establecimiento}`}
                        </div>
                      </td>
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                {/* SELECT CASCADA: CLIENTE -> ESTABLECIMIENTO */}
                ...
                <div className="col-span-2">
                  ...
                  <select required disabled={!formClienteId} value={formData.id_cliente_establecimiento} onChange={e => setFormData({...formData, id_cliente_establecimiento: e.target.value})} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-lgc-primary outline-none text-sm transition-all cursor-pointer disabled:opacity-50">
                    ...
                    {establecimientosForm.map(est => (
                       <option key={est.id_cliente_establecimiento} value={est.id_cliente_establecimiento}>
                         {est.descripcion}
                       </option>
                    ))}
                  </select>
                ...
                <div className="col-span-2">
                  <label className="text-[10px] font-bold uppercase text-slate-500 tracking-widest block mb-1">Especialidad de la Matriz *</label>
                  <select required value={formData.id_tipo_matriz} onChange={e => setFormData({...formData, id_tipo_matriz: e.target.value})} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-lgc-primary outline-none text-sm transition-all cursor-pointer">
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase text-slate-500 tracking-widest block mb-1">Fecha de Inicio *</label>
              ...
              {/* El campo Estado General fue removido como solicitaste. 
                  En Alta, viaja 1 por defecto. En Edición, se mantiene el actual. */}

              <div className="pt-4 flex gap-3">
                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-2.5 text-xs uppercase font-bold text-slate-500 hover:text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 rounded-lg transition-colors">Cancelar</button>
                <button type="submit" disabled={formLoading} className="flex-1 bg-lgc-primary hover:bg-lgc-hover text-white py-2.5 rounded-lg text-xs uppercase font-bold shadow-md disabled:opacity-50 transition-colors">Guardar Cambios</button>
              </div>
            </form>
          </div>
        </div>
